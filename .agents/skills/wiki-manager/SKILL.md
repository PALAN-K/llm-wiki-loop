...
0. **Vault Root Discovery**: Look for `./llm-wiki-loop/index.md` (or `./llm-wiki/index.md`) first. If present, the vault root is `<project>/llm-wiki-loop/`. Otherwise, if `./index.md` and `./AGENTS.md` exist at the project root, the vault root is `<project>/`. All `raw/`, `wiki/`, `index.md`, `log.md` paths operate relative to this vault root.
